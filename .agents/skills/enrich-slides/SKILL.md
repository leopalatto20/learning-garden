...
3. **Enrich each covered note**: add missing details, definitions, and exam-relevant specifics; correct errors (state what you corrected); replace vague claims with the lecture's precise wording. Add Mermaid diagrams automatically wherever a concept benefits from visual structure — for example, algorithms and pipelines, state machines, taxonomies, proofs, flows, timelines, or message sequences — even when the deck or source notes contain no diagram marker. Cite the source inline like `(slide 12)`.
4. **Seed new notes** for concepts the deck covers that no note has yet, linked into their neighbors and course map.
