...
3. **Enrich covered notes.** Add precise definitions, explanations, details, and examples. Cite each addition. Preserve the lecture version when research conflicts with it, record the conflict explicitly, and state any correction rather than silently replacing the claim.
4. **Seed missing concepts.** Create atomic notes for concepts covered by the research but absent from the garden. Add frontmatter, links explaining each relationship, required warnings, and the notes to the appropriate course map.
5. **Update affected maps.** Add missing structure and links revealed by the research; keep unrelated maps unchanged.
6. **Audit the result.** Check that every source section is enriched, represented by a new note, or explicitly out of scope; every research addition is traceable; every lecture contradiction is visible; required warnings and math delimiters are present; and affected maps link to all created or changed notes.
